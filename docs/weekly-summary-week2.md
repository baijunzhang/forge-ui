...
|------|--------------|----------------------------------|
| **Wealth Retail** | MSS→WPB→retail distribution; tokenisation to fractionalise bonds for retail | *(Business need / concern, not an AI Markets opportunity):* End-to-end matters — regulation, custody, settlement, suitability — not just UI |
| **G10 Rates Trading** | Market making + macro views + client flow + risk; IT integration & capital efficiency; eRisk client data | Client-flow analysis, scenario tools, systematise knowledge embedded in voice trading |
| **Corporate Sales** | FX fwd/swap/CCS tied to corporate commercial activity + HSBC large balance sheet | Extract RFQ from Bloomberg, route to trader, return price; needs data quality & entity mapping |
...
| **Electronic Trading / Cash Equities Execution** | High/low-touch, programme, algo; VWAP benchmarks, connectivity, analytics | Recommend execution method, flag abnormal performance, explain slippage & benchmark results |
...
|---------|-------|--------------|
| **AI Markets Desktop** | UI design & user-centred testing | My takeaway is that a good product must be thought about from the customer's perspective. Testing from the user's point of view (not just checking whether functions work) helps surface many more improvements |
