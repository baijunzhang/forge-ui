...
## 参考截图比对三:同一个 "please walk me through the repo" 任务,完整 Codex 6 张截图

这次拿到了 Codex 从触发到收尾的完整过程截图(不只是单张定格)。多数细节跟前两次比对记录的
判断一致,其中一张截图**直接印证**了"Checkpoint 2 补充规格"里推测的两层折叠效果——运行时
展开态的一长串步骤,完成后自动收起成一个带边框、带 chevron、可点击展开的胶囊
`"Worked for 49s ›"`,跟当时写的规格完全对得上,不需要改。**只记录这次新看到的、之前没写
过的四点**,同样不构成新授权、不改变 6 个 checkpoint 顺序:

1. **命令/代码输出块有语言标签 + 复制按钮的独立样式**,跟"一行工具摘要"是两种不同的展示层:
   工具调用本身还是折叠成一行(`Ran a command` / `Read files, ran a command` 这种自然语言
   摘要,不是固定的工具名分类);但当需要展示实际命令或代码内容时(比如 `git add index.html
   app.js app.css`),会用一个独立的、带语言标签(如顶部小字 "bash")+ 右上角复制图标的代码
   块呈现,不是塞进工具摘要行里。→ 供 **Checkpoint 2 / Terminal output 组件** 参考:工具
   一行摘要和"展示具体命令内容"应该是两层不同的组件,不要混在一起。

2. **行内文件引用的图标按文件类型变化**,不是统一一个图标:`.js` 文件前面是 JS 语言图标,
   `.html` 前面是 `</>` 代码图标,`.md` 文件前面是文档图标。→ 细化前面"参考截图比对二"里
   已经记录的"行内可点击代码/文件引用"这一条,到 **Checkpoint 2** 相关工作时,行内引用的
   图标应按文件扩展名区分,不要用同一个通用图标。

3. **收尾的"交付物卡片"里每一行都有文件类型标签和独立的 "Open in" 下拉**(如
   "Document · MD" + 右侧一个 "Open in ▾" 按钮),不只是文件名 + 图标。→ 细化前面已记录的
   "结构化交付物卡片"那一条,到 **Checkpoint 5** 时,卡片每行应包含:图标(按类型)、文件名、
   类型标签、独立的"用什么打开"操作入口。

4. **回复末尾有一排轻量的消息级操作**:复制、点赞、点踩、分享四个小图标 + 时间戳
   (7:55 PM),贴在最终回复正文下方,不占额外空间、不带背景框。这是之前两次比对都没记录
   过的全新元素,我们现有的 spec(composer/message turn 结构)里目前没有这一层。→ 建议在
   **Checkpoint 5(空状态/审批卡片/错误状态统一)** 时一并评估是否需要给每条 AI 回复加一排
   类似的轻量操作(至少复制是明显有用的;点赞点踩/分享要不要做可以到时候再定,不必现在
   决定),但不在当前插入新步骤或提前实现。
